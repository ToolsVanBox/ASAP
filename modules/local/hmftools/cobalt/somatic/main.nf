process HMFTOOLS_COBALT_SOMATIC {
    tag "$meta.id"
    label 'process_medium'

    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        'https://depot.galaxyproject.org/singularity/hmftools-cobalt:1.13--hdfd78af_1':
        'biocontainers/hmftools-cobalt:1.13--hdfd78af_1' }"

    input:
    tuple val(meta), path(normal_bam), path(normal_bai), path(tumor_bam), path(tumor_bai)
    path(fasta)
    path(gc_profile)
    
    output:
    tuple val(meta), path("*cobalt.gc.median.tsv") , emit: cobalt_gc_median_tsv
    tuple val(meta), path("*cobalt.ratio.median.tsv") , emit: cobalt_ratio_median_tsv
    tuple val(meta), path("*cobalt.ratio.pcf") , emit: cobalt_ratio_pcf
    tuple val(meta), path("*cobalt.ratio.tsv.gz") , emit: cobalt_ratio_tsv_gz
    path "versions.yml"                         , emit: versions

    when:
    task.ext.when == null || task.ext.when

    script:
    def prefix = task.ext.prefix ?: "${meta.id}"
    def args = task.ext.args ?: ''
    def tumor = meta.tumor_sample_id
    def reference = meta.normal_sample_id
    def gc_profile = gc_profile ? "-gc_profile ${gc_profile}" : ""

    def VERSION = '1.13' // WARN: Version information not provided by tool on CLI. Please update this string when bumping container versions.
    
    """ 
    COBALT \\
        ${args} \\
        -Xmx${task.memory.toGiga() - 1}g \\
        -tumor ${tumor} \\
        -tumor_bam ${tumor_bam} \\
        -reference ${reference} \\
        -reference_bam ${normal_bam} \\
        -output_dir ./ \\
        -threads ${task.cpus} \\
        ${gc_profile}
    
    cat <<-END_VERSIONS > versions.yml
    "${task.process}":
        cobalt: ${VERSION}
    END_VERSIONS
    """

    stub:
    def prefix = task.ext.prefix ?: "${meta.id}"
    def args = task.ext.args ?: ''
    def tumor = meta.tumor_sample_id
    def reference = meta.normal_sample_id
    def VERSION = '1.13' // WARN: Version information not provided by tool on CLI. Please update this string when bumping container versions.

    """
    touch ${reference}.cobalt.qc.median.tsv
    touch ${reference}.cobalt.ratio.median.tsv
    touch ${reference}.cobalt.ratio.pcf

    touch ${tumor}.cobalt.qc.median.tsv
    touch ${tumor}.cobalt.ratio.pcf
    touch ${tumor}.cobalt.ratio.tsv.gz

    cat <<-END_VERSIONS > versions.yml
    "${task.process}":
        cobalt: ${VERSION}
    END_VERSIONS
    """
}
