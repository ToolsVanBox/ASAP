//
// BAM TUMOR-ONLY SHORT VARIANT DISCOVERY CONTROLFREEC
...
workflow BAM_TUMORONLY_COPY_NUMBER_DISCOVERY_CONTROLFREEC {
  take:
  ...
  main:
    ...
    def ploidy = params.bam_tumoronly_copy_number_discovery_controlfreec.ploidy ?: 2
    ...
    FREEC_TUMORONLY(controlfreec_input, fasta, len, [], dbsnp, dbsnp_tbi, chr_files, mappability, [], [])
    ch_versions = ch_versions.mix( FREEC_TUMORONLY.out.versions )

    controlfreec_assesssignificance_input = FREEC_TUMORONLY.out.CNV.join( FREEC_TUMORONLY.out.ratio )
    ...
    controlfreec_makegraph_input = FREEC_TUMORONLY.out.ratio.join( FREEC_TUMORONLY.out.BAF ).map{ meta, ratio, baf -> [ meta, ratio, baf, ploidy ] }
    CONTROLFREEC_MAKEGRAPH( controlfreec_makegraph_input )
    ch_versions = ch_versions.mix( CONTROLFREEC_MAKEGRAPH.out.versions )

    CONTROLFREEC_FREEC2BED( FREEC_TUMORONLY.out.ratio )
    ch_versions = ch_versions.mix( CONTROLFREEC_FREEC2BED.out.versions )

    CONTROLFREEC_FREEC2CIRCOS( FREEC_TUMORONLY.out.ratio )
    ch_versions = ch_versions.mix( CONTROLFREEC_FREEC2CIRCOS.out.versions )

    CONTROLFREEC_MAKEKARYOTYPE( FREEC_TUMORONLY.out.ratio )
    ch_versions = ch_versions.mix( CONTROLFREEC_MAKEKARYOTYPE.out.versions )

    CONTROLFREEC_MAKEBAFPLOT( FREEC_TUMORONLY.out.BAF )
